...
| 이미지 용량 과다 (2GB+) | 의존성 중복 및 컨텍스트 과다: 불필요한 파일과 모든 종속성을 한 레이어에 `COPY`하여 용량 급증. | 멀티 스테이지 빌드: 빌드 환경과 런타임 환경을 분리하여 최종 이미지 경량화. Linux환경에서 필요한 페키지(fastapi등)만 설치, 용량은 원래의 5.78GB부터 293MB까지 감소함
### 개발 중 깨달은 점과 개념 정리
