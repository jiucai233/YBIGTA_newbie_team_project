...
## 기타
초기에는 Streamlit Cloud에 호스팅된 프론트엔드와 EC2 상의 MCP 서버를 크로스 도메인(Cross-domain) 방식으로 연동하고자 하였다. SSL/TLS 인증서 재구성을 통해 혼합 콘텐츠(Mixed Content) 차단 문제는 해결하였으나, 실제 호출 과정에서 SSE(Server-Sent Events)의 세션 상태 의존성으로 인한 문제가 발생하였다. 복잡한 네트워크 경로상에서 세션 컨텍스트(Context) 유실이 확인된 것이다.

구체적으로 MCP 서버 로그에서는 Tool 등록이 정상적으로 완료되었음을 확인하였으나, 프론트엔드 측의 요청이 활성 세션에 정확히 매칭되지 않아 'Unknown tool' 에러가 지속적으로 반환되었다. 이는 Streamlit Proxy와 AWS ALB 등 다중 프록시 계층이 도입되면서 발생하는 네트워크 불확실성에 기인한 것으로 판단하였다.

이에 따라 다중 프록시로 인한 호환성 병목 현상을 해결하고자 전체 EC2 로컬 배포 방식으로 전환하였다. 내부 네트워크 통신을 통해 크로스 도메인 이슈를 근본적으로 차단함으로써, 에이전트 호출 체인(Agent Call Chain)의 안정성을 최종적으로 확보하였다.
