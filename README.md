...
## RDS, Load Balancer
### RDS보호
RDS는 데이터베이스를 위한 서비스로, EC2와 연결이 가능하며 각종 백업, 확장, 로그 기능을 제공한다. 인프라 기초 구축 및 연결 측면에서 RDS는 매우 실용적인 도구이다. RDS는 DBMS를 기반으로 생성된 하나의 인스턴스로 볼 수 있으므로, 인바운드(Inbound)와 아웃바운드(Outbound)에 대한 제한이 존재한다. AWS에는 EC2와 RDS를 클릭 한 번으로 연결해주는 기능이 있으며, 해당 기능을 활용하였다.
...
ALB(Application Load Balancer)의 주요 기능은 크게 두 가지이다. 첫째, 여러 EC2 인스턴스로 요청을 할당(Allocate)하는 것이며, 둘째, 타겟 그룹(Target Group)과 보안 그룹(SG)을 생성 및 적용하여 EC2 인스턴스의 포트를 보호하는 것이다. 여기서 타겟 그룹이란 ALB가 요청을 전달할 대상 인스턴스의 리스트를 의미한다.
![ALB](github/ALB.png)
...
| 어려운 점| 근본 원인  | 해결 방안|
|----------------------|----------------------|----------------------|
| 대상 그룹(Target Group) 'Unused' 상태 | 가용 영역(AZ) 불일치: EC2는 `2c`에 있으나, ALB 생성 시 `2a`, `2b` 서브넷만 선택되어 트래픽 전달이 불가능하다. | 네트워크 매핑 수정: ALB 설정에서 해당 인스턴스가 포함된 모든 서브넷을 체크하여 교차 가용 영역 프로브를 활성화하였다. |
| ALB 도메인 접속 타임아웃 | 보안 그룹(SG) 설정 오류: 80(HTTP) 포트 개방 규칙을 트래픽 입구인 ALB가 아닌 RDS 보안 그룹에 잘못 적용하였다. | 3계층 분리 적용: ALB 전용 보안 그룹에서 `0.0.0.0/0` 대상 80 포트를 개방하여 외부 입구를 확보하였다. |
| 백엔드 포트 은닉 (Port Isolation) | 보안 규정상 외부에서 `IP:8000`으로 ALB를 우회하여 직접 접속하는 것을 차단해야 한다. | 보안 그룹 상호 참조 (Referencing): EC2 보안 그룹의 인바운드 규칙을 ALB 보안 그룹 ID로부터의 8000 포트 트래픽만 허용하도록 수정하였다. |
| RDS 연결 지속 타임아웃 | 소스 주소 검증 실패: 네트워크(연세대 WiFi 등) 공망 IP 변경으로 인해 기존 고정 IP 권한이 무효화되었다. | 내장 신뢰 설정: RDS 보안 그룹의 인바운드 소스를 EC2 보안 그룹 ID로 지정하여 안정적인 내원 통신을 구현하였다. |

2. Docker 이미지 배포 및 환경 최적화 (Docker & OS)
이미지 용량 과부하 및 권한 문제 등 로컬에서 클라우드로 넘어가는 과정의 최적화 단계이다.

| 어려운 점| 근본 원인 | 해결 방안  |
|----------------------|----------------------|----------------------|
| .pem 키 권한 오류 | 파일 시스템 매핑 실책: WSL 마운트 경로(`/mnt/d/`)는 Linux의 `chmod 400` 권한 설정을 완벽히 지원하지 않는다. | 경로 이전: 키 파일을 Linux 네이티브 홈 디렉토리(`~/.ssh/`)로 이동 후 권한을 수정하였다. (초기 개발 단계)|
| 반복적인 sudo 사용 | 사용자 권한 미부여: 현재 사용자가 `docker` 그룹에 포함되지 않아 매번 루트 권한이 필요하다. | 그룹 권한 부여: `sudo usermod -aG docker $USER` 실행 후 세션을 재접속하였다. |
| 이미지 용량 과다 (2GB+) | 의존성 중복 및 컨텍스트 과다: 불필요한 파일과 모든 종속성을 한 레이어에 `COPY`하여 용량이 급증하였다. | 멀티 스테이지 빌드: 빌드 환경과 런타임 환경을 분리하여 최종 이미지를 경량화하였다. Linux 환경에서 필요한 패키지(fastapi 등)만 설치하여 용량은 원래의 5.78GB부터 293MB까지 감소하였다. |
### 개발 중 깨달은 점과 개념 정리
1. 보안 그룹 참조 (Security Group Referencing) vs IP 화이트리스트
개념: 접속 허용 기준을 특정 '소스 IP(0.0.0.0/0 또는 고정 IP)'가 아닌 '소스의 신원(SG ID)'으로 설정하는 방식이다.

심층 이해: 동적인 클라우드 환경에서 IP는 유동적(예: 연세대학교 WiFi 공인 IP 변동)이지만, 보안 그룹 ID는 고유하며 정적이다. RDS 보안 그룹이 EC2 보안 그룹을 신뢰하도록 설정함으로써 '신원 계약' 기반의 내장 신뢰 체계를 구축하였다. 이를 통해 네트워크 환경 변화에 따른 연결 중단 문제를 근본적으로 해결하였다.
...
ALB (Application Load Balancer): 외부와 통신하는 유일한 관문이며, 외부로부터의 부하를 수용하고 분산한다.

EC2 (Computing Layer): 프라이빗 신뢰 사슬 뒤에 보호받는 순수 계산 계층이다.

RDS (Database Layer): 데이터의 핵심부로서 외부로부터의 직접적인 접근을 엄격히 차단한다.

논리적 폐쇄성: 포트 은닉(Port Isolation)이 이루어지지 않은 상태의 ALB 도입은 보안상 무의미함. 즉, 모든 백엔드 접점은 오직 로드 밸런서를 통해서만 연결되어야 한다.

#### 💡 프로젝트 수행을 통한 주요 인사이트
1. 학생 과제와 실무 배포의 본질적 차이
단순히 코드를 클라우드에 올리는 '학생 수준의 과제'와 실제 사용자를 대상으로 하는 '서비스 배포'는 차원이 다른 영역이다.

실제 서비스 단계에서는 단순 기능 구현을 넘어, 외부 공격 방어(Security) 및 효율적인 트래픽 분산(Traffic Distribution)이 필수적으로 고려되어야 한다.

2. 정밀 기기로서의 소프트웨어 시스템
성공적인 서비스 배포를 위해서는 전체 시스템이 마치 정교하게 설계된 정밀 기기처럼 유기적으로 맞물려 돌아가야 함을 체감하였다.

인프라의 세밀한 설정(포트 하나, 보안 그룹 규칙 하나)이 전체 서비스의 생존과 직결됨을 인식하였다.

3. 클라우드 서비스(CSP) 활용의 중요성
AWS, GCP와 같은 클라우드 서비스 제공업체는 이러한 정교한 시스템을 안정적으로 구축할 수 있도록 돕는 강력한 도구이다.

성능과 보안이 보장된 인프라를 구축하기 위해서는 클라우드 서비스의 특성을 깊이 있게 이해하고 활용하는 능력이 필수적이다.

# LangGraph State 및 Router 구현
주소:https://ybigta4th.streamlit.app/
## 1. State Class 구현 방식 (`GraphState`)
- `TypedDict`를 사용하여 그래프의 상태(State) 구조를 정의하였다.
- `user_input`: 사용자의 최신 질문을 저장한다.
- `chat_history` & `messages`: `Annotated[List[BaseMessage], operator.add]`를 사용하여 새로운 메시지가 추가될 때 기존 리스트에 덮어쓰지 않고 자동으로 합쳐지도록(Append) 구현하였다.
- `review`, `subject`, `context`, `analysis_result`: 각 노드(RAG, 검색, MCP 등)에서 처리된 결과물을 담아 다른 노드나 최종 응답 생성에 활용할 수 있도록 필드를 구성하였다.

## 2. 조건부 라우팅(Conditional Routing) 구현 방식
- `st_app/graph/router.py`의 `router` 함수를 통해 사용자의 의도(Intent)를 분류하고 다음에 실행할 노드를 결정한다.
- **LLM 기반 분류**: Upstage의 `solar-mini` 모델을 활용하여 사용자의 입력을 4가지 카테고리로 분류한다.
    - `rag_review_node`: 특정 놀이기구나 시설에 대한 리뷰/의견을 묻는 경우
    - `subject_info_node`: 일반적인 시설 정보나 위치 등을 묻는 경우
    - `mcp`: 데이터 분석, 통계, 트렌드 분석을 원하는 경우
    - `chat_node`: 일상적인 대화나 기타 질문인 경우
- 분류된 결과에 따라 LangGraph의 `add_conditional_edges`를 통해 유연하게 워크플로우를 전환하도록 설계되었다.

# MCP (Model Context Protocol) 구현
http://ybigta-1719642863.ap-southeast-2.elb.amazonaws.com/ 이는 MCP를 담긴 AWS주소다
## 1. MCP 연결 방식
MCP는 데이터 분석 서버와 메인 애플리케이션 간의 효율적인 통신을 위해 도입되었다. 구현 방식 및 특징은 다음과 같다.
- **서버 아키텍처**: `FastMCP` 프레임워크를 기반으로 구축되었으며, 서버와 클라이언트 간의 실시간 통신을 위해 SSE(Server-Sent Events) 프로토콜을 사용한다.
- **연결 메커니즘**: 메인 애플리케이션의 `mcp_node`에서 `sse_client`를 통해 MCP 서버에 접속한다. 사용자의 요청이 데이터 분석을 필요로 할 경우, LLM(Upstage Solar-mini)이 의도를 파악하여 적절한 MCP 도구를 선택하고 결과를 수신한다.
- **데이터 통합**: 카카오맵과 트립닷컴의 전처리된 데이터를 로드하여 통합한 뒤, 실시간으로 통계 분석을 수행하여 결과를 반환하는 구조를 가진다.

## 2. MCP 도구(Tools) 구성
MCP 서버는 다음과 같은 4가지 핵심 분석 도구를 제공하여 데이터 기반의 인사이트를 도출한다.
- **`get_sentiment_summary`**: 전체 리뷰 데이터의 평균 평점과 긍정 리뷰 비율을 계산하여 전반적인 만족도 요약 정보를 제공한다.
- **`analyze_crowd_impact`**: 평일과 주말의 평균 평점을 비교 분석하여, 방문 시점에 따른 만족도 차이와 인사이트를 도출한다.
- **`get_seasonal_trends`**: 월별 만족도 추이를 분석하여 사용자가 방문하기 가장 좋은 시기와 피해야 할 시기를 식별한다.
- **`extract_high_value_complaints`**: 평점이 낮고 내용이 긴 리뷰를 필터링하여 운영상의 핵심 문제점과 구체적인 개선 필요 사항을 추출한다.
![mcp image](github/mcp.png)
제시된 데이터에 근거하여 주중과 주말의 평점을 분석한 결과, 주중 평점(4.34)이 주말 평점(4.28)보다 높음을 확인하였다. 이는 `analyze_crowd_impact` 도구를 활용하여 혼잡도가 사용자 만족도에 미치는 영향을 분석한 데이터에 기반한 결과이다.

## 기타
초기에는 Streamlit Cloud(HTTPS)와 EC2 상의 MCP 서버(HTTP)를 연동하고자 하였다. 이 과정에서 HTTPS와 HTTP 간의 프로토콜 비호환성으로 인한 혼합 콘텐츠(Mixed Content) 문제가 발생하였으며, 이를 해결하기 위해 AWS Certificate Manager(ACM)를 통해 SSL/TLS 인증서를 발급하고 보안 연결을 구성하였다.

그러나 인증서 적용 후에도 실제 도구 호출 과정에서 문제가 지속되었다. 이는 SSE(Server-Sent Events)가 세션 상태에 극도로 의존하는 특성을 가졌기 때문이다. Streamlit Proxy와 AWS ALB 등 다중 프록시를 거치는 복잡한 네트워크 경로는 세션 컨텍스트(Context) 유실을 초래하였다.

결과적으로 MCP 서버 로그에서는 도구가 정상 등록된 것으로 나타나지만, 프론트엔드 요청이 활성 세션에 매칭되지 않아 'Unknown tool' 에러가 반환되는 현상이 발생하였다. 이러한 네트워크 불확실성을 완전히 제거하기 위해 전체 EC2 로컬 배포 방식으로 전환하였으며, 내부 네트워크 통신을 통해 프로토콜 및 세션 호환성 병목 현상을 근본적으로 해결하고 에이전트 호출의 안정성을 확보하였다.
